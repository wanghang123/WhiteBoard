...
import static com.yinghe.whiteboardlib.utils.CommConsts.PAGES_MAX;

/**
 * 图片Adapter
 */
public class SketchDataGridAdapter extends BaseAdapter {
    String TAG = "SketchDataGridAdapter";
    ...
    private void bindView(View view, final ViewHolder holder, final int position) {
        ...
        holder.sketchLay.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                onActionCallback.onSelectCallback(position);
            }
        });
        ...
        holder.addIV.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (onActionCallback != null && getCount() <= CommConsts.PAGES_MAX){
                    onActionCallback.onAddCallback(position);
                } else {
                    // 白板已经达到最大数量
                    String pageMax = mContext.getString(R.string.sketch_count_alert, PAGES_MAX + "");
                    Toast.makeText(mContext, pageMax, Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
